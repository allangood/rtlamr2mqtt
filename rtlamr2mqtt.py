...
rtlamr_cmd = ['/usr/bin/rtlamr', '-msgtype=%s' % rtlamr_protocol, '-format=csv', '-filterid={}'.format(meter_id)]
...
mqtt_client.publish(topic=discover_topic, payload=json.dumps(discover_payload), qos=0, retain=True)
